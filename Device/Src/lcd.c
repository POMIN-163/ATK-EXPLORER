...
uint16_t LCD_ReadPoint(uint16_t x, uint16_t y) {
        ...
        lcddev.id == 0X5510)  // 9341/NT35310/NT35510要分2次读出
    {
        ...
        b = LCD_RD_DATA();
        g = r &
            0XFF;  // 对于9341/5310/5510,第一次读取的是RG的值,R在前,G在后,各占8位
        g <<= 8;
    }
...
void LCD_Init(void) {
        ...
        if (lcddev.id != 0X9341)     // 非9341,尝试是不是6804
        {
            ...
            if (lcddev.id != 0X6804)  // 也不是6804,尝试看看是不是NT35310
            {
                ...
                if (lcddev.id != 0X5310)  // 也不是NT35310,尝试看看是不是NT35510
                {
                    ...
                    if (lcddev.id == 0x8000)
                        lcddev.id =
                            0x5510;  // NT35510读回的ID是8000H,为方便区分,我们强制设置为5510
                    if (lcddev.id !=
                        0X5510)  // 也不是NT5510,尝试看看是不是SSD1963
                    {
                        LCD_WR_REG(0XA1);
    ...
    if (lcddev.id == 0X9341 || lcddev.id == 0X5310 || lcddev.id == 0X5510 ||
        lcddev.id == 0X1963)  // 如果是这几个IC,则设置WR时序为最快
    {
        // 重新配置写时序控制寄存器的时序
        FSMC_Bank1E->BWTR[6] &= ~(0XF << 0);  // 地址建立时间(ADDSET)清零
        FSMC_Bank1E->BWTR[6] &= ~(0XF << 8);  // 数据保存时间清零
        FSMC_Bank1E->BWTR[6] |= 3 << 0;  // 地址建立时间(ADDSET)为3个HCLK =18ns
        FSMC_Bank1E->BWTR[6] |= 2
                                << 8;  // 数据保存时间(DATAST)为6ns*3个HCLK=18ns
    } else if (lcddev.id == 0X6804 ||
               lcddev.id == 0XC505)  // 6804/C505速度上不去,得降低
    {
        // 重新配置写时序控制寄存器的时序
        FSMC_Bank1E->BWTR[6] &= ~(0XF << 0);  // 地址建立时间(ADDSET)清零
        FSMC_Bank1E->BWTR[6] &= ~(0XF << 8);  // 数据保存时间清零
        FSMC_Bank1E->BWTR[6] |= 10 << 0;  // 地址建立时间(ADDSET)为10个HCLK
                                          // =60ns
        FSMC_Bank1E->BWTR[6] |=
            12 << 8;  // 数据保存时间(DATAST)为6ns*13个HCLK=78ns
    }
    printf("LCD ID:%x\r\n", lcddev.id);  // 打印LCD ID
    ...
    } else if (lcddev.id == 0x6804)  // 6804初始化
    {
        LCD_WR_REG(0X11);
        delay_ms(20);
        LCD_WR_REG(0XD0);  // VCI1  VCL  VGH  VGL DDVDH VREG1OUT power
        // amplitude setting
        LCD_WR_DATA(0X07);
        LCD_WR_DATA(0X42);
        LCD_WR_DATA(0X1D);
        ...
        LCD_WR_DATA(0X1a);
        LCD_WR_DATA(0X09);
        LCD_WR_REG(0XD2);  // Operational Amplifier Circuit Constant Current
        // Adjust , charge pump frequency setting
        LCD_WR_DATA(0X01);
        LCD_WR_DATA(0X22);
    ...
    } else if (lcddev.id == 0x9320)  // 测试OK.
    {
        ...
        LCD_WriteReg(0x0c,
                     (1 << 0));  // Extern Display Interface Contral 1.(0x0000)
        ...
        LCD_WriteReg(0x81, 0x0000);  // RAM Address Start? Partial
        // Display 1.
        LCD_WriteReg(0x82, 0x0000);  // RAM Address End-Partial Display 1.
        LCD_WriteReg(0x83, 0x0000);  // Displsy Position? Partial Display 2.
        LCD_WriteReg(0x84, 0x0000);  // RAM Address Start? Partial
        // Display 2.
        LCD_WriteReg(0x85, 0x0000);  // RAM Address End? Partial Display 2.

        LCD_WriteReg(0x90,
                     (0 << 7) | (16 << 0));  // Frame Cycle Contral.(0x0013)
    ...
    } else if (lcddev.id == 0X9331)    // OK |/|/|
    {
        ...
        // LCD_WriteReg(0x0003, 0x1030); // set GRAM write direction and
        // BGR=1.
        ...
        LCD_WriteReg(0x80, 0x0000);  // Display Position – Partial Display 1
        LCD_WriteReg(0x81, 0x0000);  // RAM Address Start – Partial Display
        // 1
        LCD_WriteReg(0x82, 0x0000);  // RAM address End - Partial Display 1
        LCD_WriteReg(0x83, 0x0000);  // Display Position – Partial Display 2
        LCD_WriteReg(0x84, 0x0000);  // RAM Address Start – Partial Display
        // 2
    ...
    } else if (lcddev.id == 0x1505)  // OK
    {
        ...
        delay_ms(50);
        LCD_WriteReg(0x0012,
                     0x011C);  // 0x011A   why need to set several times?
        ...
        LCD_WriteReg(
            0x0011,
            0x0217);  // reference voltage VC[2:0]   Vciout = 1.00*Vcivl
        LCD_WriteReg(0x0012, 0x011E);  // 0x011c  //Vreg1out = Vcilvl*1.80
        // is it the same as Vgama1out ?
        LCD_WriteReg(
            0x0013,
            0x0F00);  // VDV[4:0]-->VCOM Amplitude VcomL = VcomH - Vcom Ampl
        LCD_WriteReg(0x002A, 0x0000);
        LCD_WriteReg(0x0029,
                     0x000A);  // 0x0001F  Vcomh = VCM1[4:0]*Vreg1out gate
        // source voltage??
    ...
    } else if (lcddev.id == 0x4531)  // OK |/|/|
    {
        ...
        LCD_WR_REG(
            0xE2);  // Set PLL with OSC = 10MHz (hardware),	Multiplier N =
